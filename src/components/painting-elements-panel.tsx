...
export function PaintingElementsPanel() {
  const [items, setItems] = useState<DataPaintingElementPublic[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "Failed to load painting elements");
      setItems([]);
      setLineCount(0);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    async function bootstrapThenLoad() {
      ...
      setError(null);
      try {
        const initRes = await fetch("/api/painting-elements/init", { method: "POST" });
        const initData = (await initRes.json()) as { error?: string };
        if (!initRes.ok) {
          setError(
            initData.error ?? "Failed to initialize painting elements collection in Firestore",
          );
          setItems([]);
          setLineCount(0);
          setLoading(false);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0 space-y-1">
          <h2 className={sfSectionHeading}>Painting Elements</h2>
          <p className={sfSectionLead}>
            Read-only view of{" "}
            <code className="rounded bg-sf-page px-1 font-mono text-sm dark:bg-zinc-800">
              data_painting_elements
            </code>
            . Import from Data Import → Painting Elements (
            <code className="text-xs">Painting Elements</code> tab, columns H+).
          </p>
        </div>
      ...
      <div className={sfDataSurface}>
        {loading ? (
          ...
          <p className="p-6 text-sf-text-secondary dark:text-zinc-400">
            {items.length === 0
              ? "No painting elements yet. Run Painting Elements import on Data Import."
                    ...
                    <table className="w-full min-w-[960px] text-left text-sm">
                      <thead>
                        <tr className="border-b border-sf-border text-sf-text-secondary dark:border-zinc-700 dark:text-zinc-400">
                          ...
                          <th className="py-2 pr-3 font-medium">Litre/M2</th>
                          <th className="py-2 pr-3 font-medium">Price litre</th>
                          <th className="py-2 pr-3 font-medium">Price M2</th>
                        ...
                        {el.lines.map((line, idx) => (
                          ...
                          >
                            ...
                            <td className="py-2 pr-3 whitespace-nowrap">
                              {line.litrePerM2 == null
                                ? "—"
                                : formatQuantity(line.litrePerM2)}
                            </td>
                            <td className="py-2 pr-3 whitespace-nowrap">
                              {formatMoney(line.priceLitre)}
                            </td>
                            <td className="py-2 pr-3 whitespace-nowrap">
                              {formatMoney(line.priceM2)}
                            </td>
                            <td className="py-2 pr-3 whitespace-nowrap">
                              {formatQuantity(line.m2Multiplier)}
                            </td>
