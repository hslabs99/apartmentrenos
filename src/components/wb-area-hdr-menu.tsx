...
export function WbAreaHdrMenu({
  areaLabel,
  addObjectDisabled = false,
  addBlankLineDisabled = false,
  ...
  const menuDisabled = addObjectDisabled && addBlankLineDisabled && removeDisabled;
  ...
  return (
    <div className="relative shrink-0" ref={wrapRef}>
      <button
        type="button"
        aria-label={`Actions for ${areaLabel}`}
        aria-expanded={open}
        aria-haspopup="menu"
      ...
      {open ? (
        <div
          role="menu"
          className="absolute right-0 z-30 mt-1 min-w-[11rem] rounded-lg border border-sf-border bg-sf-surface py-1 shadow-lg dark:border-zinc-700 dark:bg-zinc-900"
        >
          <button
            type="button"
            role="menuitem"
          ...
          <button
            type="button"
            role="menuitem"
            disabled={addBlankLineDisabled}
            className="block w-full px-3 py-2 text-left text-sm font-medium text-amber-900 hover:bg-amber-50 disabled:cursor-not-allowed disabled:opacity-50 dark:text-amber-200 dark:hover:bg-amber-950/40"
            onClick={() => {
              setOpen(false);
              onAddBlankLine();
            }}
